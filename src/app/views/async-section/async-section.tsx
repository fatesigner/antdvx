...
export const AsyncSectionView = defineComponent({
  name: 'AsyncSectionView',
  setup() {
    let i = 0;
  ...
  render(ctx) {
    return (
      <PageWrapper title='Async Action'>
        <div class='tw-p-2'>
          <div class='tw-p-4 tw-space-y-2 tw-bg-white'>
            <div class='tw-p-2 tw-border tw-border-gray-300'>
              <AsyncSection
                ...
                v-slots={{
                  default({ data, reload }) {
                    return [<span>{data.text}</span>, <XButtonRefresh only-icon color='primary' size='small' type='link' handler={reload} />];
                  }
                }}
              />
            </div>
