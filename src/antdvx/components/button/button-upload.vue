<template>
  ...
  >
    <UploadOutlined v-if="!loading && (mode === 'default' || mode === 'icon')" />
    <slot>
      <span v-f="mode === 'default' || mode === 'text'">{{ $t(i18nMessages.antd.action.upload) }}</span>
    </slot>
  </XButton>
</template>
...
export default defineComponent({
  components: { XButton, Iconfont, UploadOutlined },
  props: {
    ...XButtonProps,
    accept: {
      // 'excel' | 'pdf'
